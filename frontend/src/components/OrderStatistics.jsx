...
const OrderStatistics = ({ statsUpdated }) => {
  const [stats, setStats] = useState(null);
  const accessToken = localStorage.getItem("accessToken");
  const token = `Bearer ${accessToken}`;
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(
        "http://117.103.207.132:8080/furni-shop/admin/orders/stats-orders",
        {
          headers: {
            Authorization: token,
          },
  ...
  useEffect(() => {
    fetchStats();
  }, [statsUpdated]); // Gọi lại fetchStats khi statsUpdated thay đổi
